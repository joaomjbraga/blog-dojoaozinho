...
export function PostCard({ post }: PostCardProps) {
  return (
    <Link 
      href={`/blog/${post.slug}`} 
      className="group block w-full transform transition-all duration-500 hover:-translate-y-2"
      aria-label={`Read more about ${post.title}`}
    >
      <article className="relative overflow-hidden rounded-2xl bg-green-50 dark:bg-zinc-900/90 shadow-xl hover:shadow-2xl transition-all duration-500">
        {post.coverImage && (
        ...
        <div className="absolute top-2 sm:top-3 right-2 sm:right-3 flex flex-wrap gap-1.5 sm:gap-2">
          {post.tags && post.tags.slice(0, 2).map(tag => (
            <span 
              key={tag} 
              className="rounded-full bg-white/90 dark:bg-zinc-800/90 backdrop-blur-md px-2 sm:px-3 py-0.5 sm:py-1 text-xs font-semibold text-zinc-900 dark:text-white shadow-lg"
            >
              {tag}
            </span>
          ))}
